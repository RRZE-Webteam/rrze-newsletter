/* globals rrze_email_editor_data */
...
const getSponsorFlagBlockTemplate = ( content, { textFontSize } ) => {
	return [
		'core/heading',
		assignFontSize( textFontSize, {
			className: 'rrze-sponsors-flag',
			content: `<span style="background-color:${ rrze_email_editor_data.sponsors_flag_hex };color:${ rrze_email_editor_data.sponsors_flag_text_color };font-weight:700;padding:2px 4px;text-transform:uppercase">${ content }</span>`,
			level: 6,
			fontSize: 'small',
		} ),
	];
...
const createBlockTemplatesForSinglePost = ( post, attributes ) => {
	...
	if ( hasSponsors ) {
		...
		const showSponsorFlag =
			'native' === post.meta.rrze_sponsor_sponsorship_scope ||
			post.rrze_sponsors_info.reduce( ( acc, sponsor ) => {
	...
	if ( hasSponsors && 'underwritten' !== post.meta.rrze_sponsor_sponsorship_scope ) {
		...
		if (
			...
			( 'inherit' === post.meta.rrze_sponsor_native_byline_display &&
				false ===
					post.rrze_sponsors_info.reduce( ( acc, sponsor ) => {
