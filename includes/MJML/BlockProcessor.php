...
class BlockProcessor
{
    ...
    public static function renderMjmlComponent(
        ...
        switch ($blockName) {
            ...
            case 'core/buttons':
                $markup = self::renderButtonsBlock(
                    $attrs,
                    $innerBlocks,
                    $fontFamily,
                    $availableWidth
            ...
            case 'core/button':
                $buttonAttrs = AttributeHandler::processAttributes($block['attrs'] ?? []);
                $markup = self::renderButtonBlock($buttonAttrs, $innerHtml, $fontFamily);
                $sectionAttrs = self::filterSectionAttributes($sectionAttrs);
                break;
    ...
    private static function renderButtonsBlock(
        array $attrs,
        array $innerBlocks,
        string $fontFamily,
        int $availableWidth
    ): string {
        $align = self::getButtonsAlignment($attrs);
        $markup = '';
        ...
        foreach ($innerBlocks as $buttonBlock) {
            ...
            $buttonAttrs = self::getButtonAttributes(
                $attrs,
                $buttonBlock['attrs'] ?? []
            );
            $buttonAttrs = AttributeHandler::processAttributes($buttonAttrs);
            $markup .= self::renderButtonBlock(
                $buttonAttrs,
    ...
    private static function getButtonAttributes(
        ...
        if (
            ...
            if (!empty($containerAttrs['font-size'])) {
    ...
    private static function getButtonsAlignment(array $attrs): string
    {
    ...
    private static function renderButtonBlock(
        array $attrs,
        string $innerHtml,
        string $fontFamily,
        string $align = 'left',
        ...
        $content = self::parseButtonContent($attrs, $innerHtml);
        if ($content['text'] === '') {
            return '';
        }

        $buttonAttrs = self::buildButtonAttributes(
            $attrs,
            $content,
            $fontFamily,
            $align,
     ...
     */
    private static function parseButtonContent(array $attrs, string $innerHtml): array
    {
        $content = [
     ...
     */
    private static function buildButtonAttributes(
        array $attrs,
        array $content,
        string $fontFamily,
        string $align,
        ...
        $isOutline = str_contains((string) ($attrs['className'] ?? ''), 'is-style-outline');
        $textColor = $attrs['color'] ?? ($isOutline ? '#32373c' : '#ffffff');
        $backgroundColor = $attrs['background-color'] ?? '#32373c';

        $buttonAttrs = [
            'align' => $align,
            'background-color' => $isOutline ? 'transparent' : $backgroundColor,
            'color' => $textColor,
            'font-family' => $fontFamily,
            'font-size' => $attrs['font-size'] ?? '16px',
            'font-weight' => $attrs['style']['typography']['fontWeight'] ?? 'normal',
            'inner-padding' => self::getButtonInnerPadding($attrs),
            'padding' => '0 0 10px',
            'border-radius' => self::getButtonBorderRadius($attrs),
        ];

        if ($content['url'] !== '') {
            $buttonUrl = self::sanitizeButtonUrl($content['url']);
            if ($buttonUrl !== '') {
        ...
        $border = self::getButtonBorder($attrs, $isOutline, $textColor);
        if ($border !== null) {
            $buttonAttrs['border'] = $border;
        }

        $width = self::getButtonWidth($attrs, $availableWidth);
        if ($width !== null) {
        ...
        foreach ([
            ...
            if (!empty($attrs['style']['typography'][$styleKey])) {
                $buttonAttrs[$mjmlAttribute] = $attrs['style']['typography'][$styleKey];
            }
        }
    ...
    private static function getButtonInnerPadding(array $attrs): string
    {
        ...
        if (is_string($padding) || is_numeric($padding)) {
            $value = self::normalizeButtonSpacingValue($padding);
            return $value . ' ' . $value;
        }
        ...
        return implode(' ', [
            self::normalizeButtonSpacingValue($padding['top'] ?? $vertical),
            self::normalizeButtonSpacingValue($padding['right'] ?? $horizontal),
            self::normalizeButtonSpacingValue($padding['bottom'] ?? $vertical),
            self::normalizeButtonSpacingValue($padding['left'] ?? $horizontal),
        ]);
    }

    private static function normalizeButtonSpacingValue($value): string
    {
    ...
    private static function getButtonBorderRadius(array $attrs): string
    {
        ...
        return str_contains((string) ($attrs['className'] ?? ''), 'no-border-radius')
            ? '0'
            : '4px';
    }

    private static function getButtonBorder(
        array $attrs,
        bool $isOutline,
        string $textColor
    ): ?string {
        ...
        $color = $border['color'] ?? $attrs['border-color'] ?? ($isOutline ? $textColor : null);
    ...
    private static function getButtonWidth(array $attrs, int $availableWidth): ?string
    {
        ...
        $pixels = max(1, (int) round($availableWidth * $percentage / 100));
        return $pixels . 'px';
    }
