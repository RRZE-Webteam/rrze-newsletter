...
const getAuthorBlockTemplate = (post, { textFontSize, textColor }) => {
    const { newsletter_author_info } = post;

    if (
        Array.isArray(newsletter_author_info) &&
        newsletter_author_info.length
    ) {
        const authorLinks = newsletter_author_info.reduce(
            (acc, author, index) => {
                const { author_link, display_name } = author;

                if (author_link && display_name) {
                    const comma =
                        newsletter_author_info.length > 2 &&
                        index < newsletter_author_info.length - 1
                            ? _x(
                                  ",",
                            ...
                            : "";
                    const and =
                        newsletter_author_info.length > 1 &&
                        index === newsletter_author_info.length - 1
